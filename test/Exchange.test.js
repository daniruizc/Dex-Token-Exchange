...
contract('Exchange', ([deployer, feeAccount, user1, user2]) => {
	...
	describe('depositing Ether', () => {
		...
		it('emits a Deposit event', async () => {
			...
			event.token.toString().should.eq(ETHER_ADDRESS, 'ether address is correct');
	...
	describe('depositing tokens', () => {
		...
		describe('success', () => {
			...
			it('tracks the token deposit', async () => {
				...
				// Check balance of user in exchange
	...
	describe('withdrawing tokens', () => {
		...
		describe('success', () => {
			...
			it('emits a Withdraw event', async () => {
				...
				log.event.should.eq('Withdraw');
				const event = log.args;
				event.token.toString().should.eq(token.address, 'ether address is correct');
				event.user.toString().should.eq(user1, 'user is correct');
				event.amount.toString().should.eq(amount.toString(), 'amount is correct');
				event.balance.toString().should.eq('0', 'balance is correct');
			});
		...
		describe('failure', () => {
			...
			it('rejects Ether withdraws', async () => {
				await exchange.withdrawToken(ETHER_ADDRESS, ether(10), { from: user1 }).should.be.rejectedWith(EVM_REVERT);
			});

			it('fails for insufficient balances', async () => {
				await exchange.withdrawToken(token.address, ether(10), { from: user1 }).should.be.rejectedWith(EVM_REVERT);
			});
		});
